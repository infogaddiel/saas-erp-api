...
import { create, exportExcel, getById, list, remove, update,dropdown } from './invoiceController';
import validateRequest from '../middlewares/validateRequest';
import { createInvoiceSchema, dropdownInvoiceSchema, idParamSchema, listInvoicesSchema, updateInvoiceSchema } from './invoiceValidator';

const router = Router();

router.post('/', validateRequest(createInvoiceSchema, 'body'), create);
router.get('/', validateRequest(listInvoicesSchema, 'query'), list);
router.get('/dropdown', validateRequest(dropdownInvoiceSchema, 'query'), dropdown);
