...
export const list = async (req: Request, res: Response) => {
  try {
    ...
    const customer_name = (req.query.customer_name as string) || undefined;
    ...
    console.error('List invoices controller error:', error);
    return res.status(500).json({ success: false, message: 'An error occurred' });
  }
};
...
export const update = async (req: Request, res: Response) => {
  try {
    const id = parseInt(req.params.id, 10);
    const result = await updateInvoice(id, req.body);

    if (!result.success) {
      const statusCode = (result as any).statusCode ?? (result.message === 'Invoice not found' ? 404 : 500);
      return res.status(statusCode).json(result);
    }
